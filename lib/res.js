...
const { assign, capitalize, getCache, getResCookies } = require("./util");
...
ServerResponse.prototype.get = function get(field) {
    return this.getHeader(typeof field === "string" ? field : String(field));
};
...
ServerResponse.prototype.redirect = function redirect(url, code = 302) {
    this.code = code;
    this.location = url === -1 ? this.req.referer : url;
    this.writeHead(code);
    this.end();
}
...
ServerResponse.prototype.send = function send(data) {
    if (this.req.method == "HEAD" || data === null || data === undefined) {
        this.end();
        return;
        ...
        if (!this.type) {
            ...
            } else if (/^\s*<!DOCTYPE\s+/i.test(data)) {
                this.type = "text/html";
            } else if (/^\s*<?xml\s+/.test(data)) {
                this.type = "application/xml";
            } else {
                let match = data.match(/^\s*<([a-z\-])/i),
                    tag = match && match[1];
                if (tag) {
                    this.type = HtmlTags.includes(tag) ? "text/html" : "application/xml";
                } else {
...
ServerResponse.prototype.sendFile = function sendFile(filename, cb = null) {
    ...
    sendStream(this.req, filename).on("error", e => {
        err = e;
        this.end();
    }).on("end", () => {
        if (err) {
...
Object.defineProperties(ServerResponse.prototype, {
    ...
    type: { // Set/Get `Content-Type` without `charset`.
        get() {
            ...
            return type && type.split(/\s*;\s*charset=/)[0];
        },
        set(type) {
    ...
    charset: { // Set/Get `Content-Type` only with `charset`.
        get() {
            let type = this.headers["content-type"]
            return type && type.split(/\s*;\s*charset=/)[1];
        },
        set(charset) {
    ...
    vary: { // Set/Get `Vary`.
        ...
        set(v) {
            this.headers.vary = Array.isArray(v) ? v.join("; ") : v;
        }
    },
    ...
    keepAlive: {
        ...
        set(v) {
            this.headers.connection = v ? "keep-alive" : "close";
        }
    },
    ...
    modified: {
        get() {
            ...
            if ('GET' !== this.req.method && 'HEAD' !== this.req.method)
                return true;
...
module.exports = (options, res) => {
    ...
    assign(res, "headers", new Proxy(headers, {
    ...
    assign(res, "cookies", new Proxy(cookies, {
        set(target, name, value) {
            var cookie;
