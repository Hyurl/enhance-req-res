...
const htmlRE = /^\s*<!DOCTYPE\s+/i;
const xmlRE = /^\s*<?xml\s+/i;
const tagRE = /^\s*<([a-z\-:])/i;
...
ServerResponse.prototype.send = function send(data) {
    if (this.req.method == "HEAD" || data === null || data === undefined) {
        ...
        if (!this.type) {
            ...
            } else if (htmlRE.test(data)) {
                this.type = "text/html";
            } else if (xmlRE.test(data)) {
                this.type = "application/xml";
            } else {
                let match = data.match(tagRE),
                    tag = match && match[1];
                if (tag) {
...
Object.defineProperties(ServerResponse.prototype, {
    ...
    charset: { // Set/Get `Content-Type` only with `charset`.
        ...
        get() {
            return this.headers["content-type"] &&
                this.headers["content-type"].split(/;\s*charset=/)[1];
        },
        set(charset) {
