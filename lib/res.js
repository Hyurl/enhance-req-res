...
const { getCache, getResCookies } = require("./util");
...
class Response extends EventEmitter {
     ...
     * @returns {void}
     */
    cookie(name, value = undefined, opts = {}) {
        if (value === undefined) {
     ...
     * @returns {void}
     */
    auth(realm = "HTTP Authentication") {
     ...
     * @returns {void}
     */
    download(filename, newName = undefined, cb = null) {
    ...
    /**
     * Set/Get `Content-Type` without `charset`.
     * @returns {string}
     */
    get type() {
    ...
    /**
     * Set/Get `Refresh` in a number of seconds.
    ...
    res.cookies = new Proxy(cookies, {
        set(target, name, value) {
            ...
            if (cookie.maxAge !== undefined) {
                cookie.expires = new Date(Date.now() + cookie.maxAge * 1000).toUTCString();
            }
